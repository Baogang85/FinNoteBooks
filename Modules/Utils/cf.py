...
def cf_normal(u, mu, sig):
    """
    Characteristic function of a normalrandom variable.
    """
...
def cf_mert(u, t, mu, sig, lam, mu_j, sig_j):
    """
    ...
    """
    return np.exp(t * (1j * u * mu - 0.5 * u**2 * sig**2 + lam * 
                       (np.exp(1j * u * mu_j - 0.5 * u**2 * sig_j**2) - 1)))
def cf_vg(u, t=1, mu=0, theta=-0.1, sigma=0.2, kappa=0.1):
    ...
    """
    return np.exp(t * (1j * mu * u - np.log(1 - 1j * theta * kappa * u 
...
def cf_nig(u, t=1, mu=0, theta=-0.1, sigma=0.2, kappa=0.1):
    ...
    """
    return np.exp(
        t * (1j * mu * u + 1 / kappa - np.sqrt(1 - 2j * theta * kappa * u 
